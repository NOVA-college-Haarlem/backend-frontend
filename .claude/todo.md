...
- [ ] In de praktijk checken of 1 week per sprint haalbaar is voor de projectlessen van zwsd25f Blok 7 (Scrum-opzet) - eerste keer uitproberen kan aanleiding geven tot bijstelling van de sprintlengte of het aantal sprints
...
- [ ] Boulder Base (`starter-website-2/`) op een later moment ook naar een eigen GitHub-repo verplaatsen (RetroPixel is al verplaatst, zie Afgerond) en de FEO/JS-hoofdstukken 4-6 dan op dezelfde manier bijwerken
...
- [x] Scrum geïntroduceerd voor de zwsd25f Blok 7-projectlessen (8 weken: Sprint 1-3 → Projectweek 1 + tussentijds assessment → Sprint 4-6 → Projectweek 2): nieuwe `Scrum Introductie.md` (dedicated les: begrippen, rollen, ceremonies, taakbord, Sprint 1 plannen) en `Sprintplanning Projectlessen.md` (volledig 8-weken-schema + het tussentijds assessment) in `projectlessen/`; alle 5 projectbeschrijvingen kregen een kant-en-klare sprintbacklog (user stories per sprint, afgeleid van de bestaande functionele eisen); `curriculum-jaar 2 - zwsd25f.md` §4 bijgewerkt om dit te beschrijven
- [x] RetroPixel (zwsd26f Blok 3 Hoofdstuk 1-3) verplaatst naar eigen repo [NOVA-college-Haarlem/RetroPixel](https://github.com/NOVA-college-Haarlem/RetroPixel): FEO- en JS-hoofdstukken bijgewerkt naar fork+clone-workflow, overzichtsdocumenten bijgewerkt, lokale `starter-website/`-map verwijderd (Boulder Base/`starter-website-2/` blijft nog lokaal, zie Open)
